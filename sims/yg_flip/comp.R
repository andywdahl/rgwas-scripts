...
allp	<- array( NA,
	dim=c(				 2					, 4				, 3													,Me			, length(types)	, length(sig2hets) ),
	dimnames=list( fliptypes	, snptypes, c('falsex','quant','bin')	,methods, types					, sig2hets )
)
allt	<- nruns	<- array( NA,
		...
		for( fliptype in fliptypes )
{
		...
		for( pp in c('falsex','quant','bin') )
	{
		...
		allp	[fliptype,xx,pp,,type,sig2.i]	<- apply( pvals[snps,phens,,,drop=F], 3	, function(x) mean( x<.01, na.rm=T )	)
	}
