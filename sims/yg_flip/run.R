rm( list=ls() )
source( '../code/main.R' )
source( '../code/sim_fxn.R', chdir=T )
load( '../Rdata/setup.Rdata' )
plotmeth	<- c( 'mfmr_fast', 'cca-Y',	'mvgmmq' ) #'mfmr+', #'allgmmq',
fliptypes	<- c( 'g->y', 'y->g' )

it	<- as.numeric( commandArgs(TRUE)[[1]] )
set.seed( it )
for( method in plotmeth )
	for( type in types )
		for( sig2.i in sample(length(sig2hets)) )
			for( fliptype in fliptypes )
{
	savefile	<- paste0( 'Rdata/',	sig2.i, '_', method, '_', type, '_', fliptype, '_', it, '.Rdata'	)
	sinkfile	<- paste0( 'Rout/' ,	sig2.i, '_', method, '_', type, '_', fliptype, '_', it, '.Rout'		)
	if( file.exists(savefile) | file.exists(sinkfile) ) next
	print( sinkfile )
	sink( sinkfile )
	try({
		simdat	<- sim_fxn( Ns[type], P=P, P_bin=P_bin, asc=F,
		K=2, zbal=F, sig2E=0,
		S=12, S_hom=4, S_het=4,
		sig2homs[sig2.i], sig2het=0,
		Etype='baseline', z_prev=.3, corrtype='Wi', AR_rho=NA, seed=it )

		if( fliptype ==  'g->y' ){
			simdat$Y	<- cbind( simdat$Y, simdat$G[,5] )
			simdat$G	<- simdat$G[,-5]
		} else if( fliptype == 'y->g' ){
			simdat$G	<- cbind( simdat$G, simdat$Y[,P] )
			simdat$Y	<- simdat$Y[,-P]
		}

		runtime	<- system.time(
			out	<- snphet_main( Y=simdat$Y, G=simdat$G, method=method, K_em=2, z=simdat$z, P_bin=P_bin )
		)[3]
		cat( 'runtime:', runtime, '\n' )
		gc()

		save( runtime, out, file=savefile )
		rm( simdat, out )
	})
	sink()
}
warnings()
