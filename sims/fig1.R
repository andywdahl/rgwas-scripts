rm( list=ls() )
load( 'Rdata/setup.Rdata' )
load( 'Rdata/out.Rdata' )
x			<- log10( Ns )

pdf( '~/figs/mfmr/Fig1.pdf', width=23, height=5.9 )
layout( cbind( rbind(
	c( 1, 3, 4, 5, 2, 6 ),
	c( 7, 14+c(1:3,5,4) ),
	8+c( 1, 3, 4, 5, 2, 6 )
),8), widths=c( 1.7, rep( 6, 3 ), 2.4, 6, 4.8 ), heights=c( 1.0, 5.3, 1.8 ) )

for( xxx in c(4,6,5,3) ){
	print( c( 'base','base,bin','base,cc','no_asc','no_asc,bin','altmeth' )[xxx] ) 

	if( xxx %in% c(1,2,3,4,5) ){
		meths	= c( 'mfmr_fast', 'mvgmmq', 'cca-Y'	, 'oracle')
		cols	= c(  1					, 2				,  4		 	,	3				)
		leg		= c( 'MFMR'			, 'GMM'		, 'CCA'		,	'Oracle')
	} else if( xxx == 6 ){
		meths	= c( 'mfmr+',	'allgmmq'		, 'pheno_pc', 'geno_pc'	, 'geno_pc_disc')
		cols	= c( 'brown',	'rosybrown1', 5					, 'orchid1'	, 'purple3'			)
		leg		= c( 'MFMR+',	'GMM+'			, 'Y PC'		, 'G PC'		, 'G PC+disc'		)
	}
	names( cols )	<- meths

	all_pvals	<- array( NA,
		dim=c(					4													,length(meths), length(x)	, 2 ),
		dimnames=list( c('null','K=1','hom','het'),meths				, x					, sig2hets )
	)

	dir		<- c( 'quant', 'bin', 'cc', 'quant', 'bin', 'quant' )[xxx]
	dir1	<- ifelse( xxx %in% 4:6, 'no_asc'				, 'base' )
	dir2	<- ifelse( xxx %in% 4:6, 'no_asc_nohet'	, 'nohet' )

	all_pvals[c('null','hom','het'),,,]	<- pvals[dir1,c( 'Null', 'Hom', 'Het' ),dir,meths,,]
	all_pvals['K=1',,,]									<- pvals[dir2,'Hom'										 ,dir,meths,,]

	print(cbind( nruns[dir1,meths,,1],nruns[dir2,meths,,1]))

	if( 'oracle' %in% meths )
		all_pvals['K=1','oracle',,]	<- NA

	##### top
	par( mar=rep(0,4) )
	plot.new()
	plot.new()
	for( zzz in 1:4 ){
		plot.new()
		text( .5, .5, cex=4.7, lab=c( 'Null SNPs', 'Hom SNPs (K=1)', 'Hom SNPs', 'Het SNPs' )[zzz] )
	}
	plot.new() ##### left

	##### right
	par( mar=c( 10, 0, 10, 0 ) )
	plot.new()
	legend( 'center', fill=cols, leg=leg, cex=c(5,4,4,5,5,4)[xxx], border=F, bty='n' )#, horiz=T )

	##### bottom
	par( mar=rep(0,4) )
	plot.new()
	plot.new()
	par( mar=c( 1.0, 2, 1.0, 2.0 )/2 )
	for( zzz in 1:4 ){
		plot(range(x)+c(-.1,.1),0:1,type='n',axes=F,ylab='',xlab='')
		text( mean(range(x)), .25, cex=3.5, lab='Sample Size (N)' )
		axis( 1, cex.axis=2.4,  at=x, line=-9.9, lab=F )
		axis( 1, cex.axis=2.4,  at=x, line=- 9.7, lab=c( '1,000', '3,000', '10,000', '30,000' ), padj=.9, tick=F )
	}
	##### meat
	par( mar=c( 1.0, 1, 1.0, 1.0 )/2 )
		for( zzz in 1:4 )
	{
		allys	<- all_pvals[zzz,,,]
		if( zzz != 4 ){
			ylim	<- c( -2.2, 0 )
			ats	<- c(.01	,.05	,.2		,1)
			ats	<- log10(ats)
			labs<- c('.01','.05','.2'	,'1' )
			allys	<- log10(allys)
		} else {
			ylim	<- c( 0, 1 )
			ats		<- c(0,.25,.5	,.75,1)
			labs	<- ats
		}

		plot( range(x)+c(-.1,.1), ylim, type='n', axes=F, ylab='', main='', xlab='' )
		box()
		if( xxx == 4 )
		legend( 'topleft', bty='n', cex=4, leg=letters[zzz], adj=c(2.5,-.3) )

		abline( a=-2	, b=0, col='lightgrey', lty=3, lwd=8 )
		if( zzz %in% c(1,4) ){
			axis( 2, cex.axis=2.7, padj=-.3	, at=ats			, lab=labs )
			mtext( side=2, line=6.2, cex=2.3 , ifelse( zzz == 1, 'False Positive Rate', 'True Positive Rate' ) )
		}

		for( meth in rev(meths) )
			for( sig2.i in 1:2 )
		try({
			lines(	x, allys[meth,,sig2.i], col=cols[meth], lty=c(1,2)[sig2.i], lwd=ifelse( meth %in% c( 'oracle', 'mfmr3' ), 5, 3 ) )
			points(	x, allys[meth,,sig2.i], col=cols[meth], pch=18-sig2.i, cex=6 )
		})
	}
	plot.new()

}
dev.off()
