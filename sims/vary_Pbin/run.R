			...
			for( truehet in c('', '_K1') )
{
	...
	sink( sinkfile )
	try({
		simdat	<- sim_fxn( N=1e4, P=30, P_bin=P_bins[type], asc=FALSE,
		K=2,
		sig2hom=sig2homs[sig2.i],
		sig2E=	ifelse( truehet=='', .1		          	, 0 ),
		sig2het=ifelse( truehet=='', sig2hets[sig2.i]	, 0 ),
		S=12, S_hom=4, S_het=4,
		Etype='baseline', z_prev=.3, corrtype='Wi', AR_rho=NA, seed=it )

		runtime	<- system.time(
			out	<- snphet_main( Y=simdat$Y, G=cbind( simdat$G ), method=method, K_em=2, z=simdat$z, P_bin=P_bins[type] )
		)[3]
		cat( 'runtime:', runtime, '\n' )
