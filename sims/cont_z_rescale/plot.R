...
pdf( '~/figs/mfmr/SFig8b.pdf', width=23, height=5.9 )
layout( cbind( 1, 2+rbind( 1:3, 7:9, 4:6 ), 2 ), widths=c( 1.9, rep( 6, 3 ), 3.9 ), heights=c( 1.0, 5.3, 1.8 ) )
...
for( zzz in 1:3 ){
	plot.new()
	text( .5, .5, cex=3.1, lab=c( 'Null SNPs', 'Hom. SNPs', 'Het. SNPs')[zzz] )
}
...
for( zzz in 1:3 ){
	...
	box()
	#legend( 'topleft', bty='n', cex=4, leg=letters[3+zzz], adj=c(2.5,-.3) )
